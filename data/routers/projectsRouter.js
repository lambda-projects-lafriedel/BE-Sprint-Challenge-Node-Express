  ...
  Projects.getProjectActions(id)
    .then(actions => {
      if (actions.length > 0) {
        ...
        res
          .status(404)
          .json({ error: "A project with the specified ID does not exist." });
      }
    })
    .catch(err => {
      res
        .status(500)
        .json({
          error: "There was an error retrieving the actions for the project."
        });
    });
});
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!name || !description) {
      res
        .status(400)
        .json({ message: "Both a project name and description are required." });
    } else {
